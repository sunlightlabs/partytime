...
class AutocompleteModelAdmin(admin.ModelAdmin):
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        ...
        if isinstance(db_field, models.ManyToManyField)and db_field.name in self.related_search_fields:
            kwargs['widget'] = ManyToManySearchInput(db_field.rel, self.related_search_fields[db_field.name])
            db_field.help_text = ''
...
class AutocompleteModelAdmin(admin.ModelAdmin):
	...
	def __call__(self, request, url):
		...
		return super(AutocompleteModelAdmin, 
self).__call__(request, url)
	...
	def search(self, request):
		...
		#	Searches in the fields of the given related 
model and returns the 

		#	result as a simple string to be used by the 
jQuery Autocomplete plugin
		...
		if search_fields and app_label and model_name and query:
			...
			def construct_search(field_name):
				...
				# use different lookup methods depending 
on the notation
...
db_field.name in self.related_search_fields:
			...
			kwargs['widget'] = 
ForeignKeySearchInput(db_field.rel,
...
self.related_search_fields[db_field.name])
			...
			# Don't wrap raw_id fields. Their add function 
is in the popup window.

			if not db_field.name in self.raw_id_fields:

				# formfield can be None if it came from 
a OneToOneField with
...
db_field.name in self.related_search_fields:
			...
			kwargs['widget'] = 
ManyToManySearchInput(db_field.rel,
...
force_unicode(obj)}
		...
		# Here, we distinguish between different save types by 
checking for
		...
		if request.POST.has_key("_continue"):
			...
			self.message_user(request, msg + ' ' + _("You 
may edit it again below."))

			if request.POST.has_key("_popup"):

				post_url_continue += "?_popup=%s" % 
request.POST.get('_popup')
...
type="text/javascript">opener.dismissAutocompletePopup(window, "%s", 
"%s");</script>' % (escape(pk_value), escape(obj)))
...
may add another %s below.") % force_unicode(opts.verbose_name)))
			...
			# Figure out where to redirect. If the user has 
change permission,

			# redirect to the change-list page for this 
object. Otherwise,
...
(rel_to._meta.app_label, rel_to._meta.object_name.lower())
		...
		output = [self.widget.render(name, value, *args, 
**kwargs)]

		if rel_to in self.admin_site._registry: # If the related 
object has an admin interface:

			# TODO: "id_" is hard-coded here. This should 
