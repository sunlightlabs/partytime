...
def main():
    ...
    try:
        cursor.execute("SELECT id, ifnull(venue_name,'') venue_name,address1,address2,city,state,zipcode, ifnull(latitude,'') latitude,ifnull(longitude,'') longitude FROM publicsite_venue order by venue_address")
    except:
        pass
    rows = cursor.fetchall()
    newrow = ["id","venue_name","Venue_Address1","Venue_Address2","Venue_City","Venue_State","Venue_Zipcode","latitude","longitude"]		
    newrowt = ["key","venue_name","Venue_Address1","Venue_Address2","Venue_City","Venue_State","Venue_Zipcode","latitude","longitude"]																																																																																																																																																																																														
