...
def main():
    ...
    try:
        cursor.execute("SELECT pe.id _id,IFNULL(start_date,'') Start_Date,IFNULL(end_date,'') End_Date,IFNULL(Start_Time,'') Start_Time,IFNULL(end_time,'') End_Time,  entertainment,venue_name,address1,address2,city,v.state,zipcode,website,concat(ifnull(v.latitude,''),';',ifnull(v.longitude,'')) LatLong,Contributions_Info,Make_Checks_Payable_To,Checks_Payable_To_Address,Committee_Id,RSVP_Info,Distribution_Paid_for_By FROM publicsite_event pe left join publicsite_venue v on (v.id = pe.venue_id)  WHERE (pe.status='' or pe.status is null)  group by pe.id")
    except:
        pass
    rows = cursor.fetchall()
    newrow =["_id","Start_Date","End_Date","Start_Time","End_Time","Entertainment","Venue_Name","Venue_Address1","Venue_Address2","Venue_City","Venue_State","Venue_Zipcode","Venue_Website","LatLong","Contributions_Info","Make_Checks_Payable_To","Checks_Payable_To_Address","Committee_Id","RSVP_Info","Distribution_Paid_for_By","Tags"]																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																			
